...
export default React.memo(function RejectProjectDialog() {
	...
	const open = React.useMemo(() => {
		return dialog.show && dialog.type === "reject_project";
	...
	const submit = async () => {
		...
		setIsLoading(true);
		try {
			await updateProjectStatus({ project_id: id, status: "rejected" });
	...
	return (
		<AppDialog
			...
			description="Are you sure you want to reject this project?"
			...
			footer={
				!isFetching && (
					<div className="flex justify-center w-full gap-10 pt-5">
						...
						<Button variant="destructive" className="w-1/2" onClick={submit} disabled={isLoading}>
